...
msz <- function(M,N,K){
    ...
    print(microbenchmark(A%*%B));print(microbenchmark(run(m,A,B)))
}

mT <- function(M,N,K){
    A<-matrix(runif(M*N,0,1),M);B<-matrix(runif(K*N,0,1),N)
    mT<-jit(glue::glue("[(x::(Arr ({M}×{N}) float))%.|:(y::Arr ({K}×{N}) float)]"))
    print(microbenchmark(A%*%t(B)));print(microbenchmark(run(mT,A,B)))
}
msz(64,64,64);mT(64,64,64);mT(1024,1024,1024)
