...
infixl 9 $$
($$) :: E (T a) -> E (T a) -> E (T a)
f$$x | Arrow _ tC <- eAnn f = EApp tC f x
...
optA (Builtin ty Dot)      | Arrow tA (Arrow _ tN) <- ty = do
    ...
    let n3=tN~>tN~>tN
    let op=λ x₀ (λ y₀ (Builtin n3 Times $$ Var tN x₀ $$ Var tN y₀))
        zop=λ x (λ y (λ z (EApp tN (Builtin n3 Plus$$Var tN x) (Builtin n3 Times$$Var tN y$$Var tN z))))
    pure $ Lam ty a (λ b (Id tN $ FoldOfZip op zop [Var tA a, Var tA b]))
optA (Builtin ty C)        | Arrow fTy (Arrow gTy (Arrow tC _)) <- ty = do
    f <- nextU "f" fTy; g <- nextU "g" gTy; x <- nextU "x" tC
    pure $ Lam ty f (λ g (λ x (Var fTy f$$(Var gTy g$$Var tC x))))
    ...
    case e' of
        ...
        (EApp _ (EApp _ (EApp _ (Builtin _ Zip) f) xs) ys)
            ...
            , Arrow _ (Arrow _ cod) <- eAnn op -> do
                ...
                let vx0 = v x0; vx1 = v x1; vx2 = v x2
                    vx0' = v x0'; vx1' = v x1'
...
optA (EApp l0 (EApp _ (Builtin _ Map) f) (EApp _ (EApp _ (Builtin _ Map) g) xs))
    | (Arrow gDom _) <- eAnn g = do
        ...
        let vx=Var gDom x
            fog=λ x (f'$$(g'$$vx))
        pure (Builtin (eAnn fog~>eAnn xs'~>l0) Map $$ fog $$ xs')
...
optA (EApp l (EApp _ (EApp _ (Builtin _ Zip) op) xs) (EApp _ (EApp _ (Builtin _ Map) g) ys))
    ...
    , Arrow dom0 (Arrow _ cod) <- eAnn op = do
        ...
        let vx0 = Var dom0 x0; vx1 = Var dom1 x1
            opTy = dom0 ~> dom1 ~> cod
            op' = λ x0 (λ x1 (opA $$ vx0 $$ (g'$$vx1)))
        pure (Builtin (opTy~>eAnn xs'~>eAnn ys'~>l) Zip $$ op' $$ xs' $$ ys')
optA (EApp l (EApp t0 (EApp t1 (Builtin bt b@FoldS) op) seed) arr) = do
    ...
    case arr' of
        (EApp _ (EApp _ (EApp _ (Builtin _ Zip) f) (EApp _ (EApp _ (EApp _ (Builtin _ Gen) gseed0) u0) n)) (EApp _ (EApp _ (EApp _ (Builtin _ Gen) gseed1) u1) _))
            ...
            z <- nextU "z" tC; y₀ <- nextU "y₀" u0Ty; y₁ <- nextU "y₁" u1Ty
            let opZ=λ z (λ y₀ (λ y₁ (opA $$ Var tC z $$ EApp tC (f $$ v y₀) (v y₁))))
            pure $ Id l $ U2 [gseed0, gseed1] [λ x0 (EApp u0Ty u0 (Var u0Ty x0)), λ x1 (EApp u1Ty u1 (Var u1Ty x1))] seed' opZ n
        ...
        (EApp _ (EApp _ (EApp _ (Builtin _ Zip) f) xs) ys)
            ...
            , Arrow _ (Arrow _ cod) <- eAnn op -> do
                ...
                let vx0 = Var cod x0; vx1 = Var dom0 x1; vx2 = Var dom1 x2
                    op' = λ x0 (λ x1 (λ x2 (EApp cod (opA $$ vx0) (EApp dom2 (f $$ vx1) vx2))))
        ...
        (EApp _ (EApp _ (EApp _ (Builtin _ Gen) u) f) n) -> do
            x <- nextU "x" uTy
            pure $ Id l $ U2 [u] [λ x (EApp uTy f (Var uTy x))] seed' opA n
          where uTy = eAnn u
