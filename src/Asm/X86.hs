...
data FX86Reg = XMM0 | XMM1 | XMM2 | XMM3 | XMM4 | XMM5 | XMM6 | XMM7 | XMM8 | XMM9 | XMM10 | XMM11 | XMM12 | XMM13 | XMM14 | XMM15
             deriving (Eq, Ord, Enum)

data F2X86 = YMM0 | YMM1 | YMM2 | YMM3 | YMM4 | YMM5 | YMM6 | YMM7 | YMM8 | YMM9 | YMM10 | YMM11 | YMM12 | YMM13 | YMM14 | YMM15
           deriving (Eq, Ord, Enum)
...
instance (Pr reg, Pr freg) => Pr (X86 reg freg a) where
    pr (Label _ l) = prLabel l <> ":"
    pr isn = i4 (pp isn) where
      pp (J _ l)                       = "jmp" <+> prLabel l
      pp (CmpRR _ r0 r1)               = "cmp" <+> pr r0 <> "," <+> pr r1
      pp (MovRR _ r0 r1)               = "mov" <+> pr r0 <> "," <+> pr r1
      pp (MovRL _ r l)                 = "mov" <+> pr r <> "," <+> "arr_" <> decimal l
      pp (MovRI _ r i)                 = "mov" <+> pr r <> "," <+> decimal i
      pp (XorRR _ r0 r1)               = "xor" <+> pr r0 <> "," <+> pr r1
      pp (MovqXR _ r0 r1)              = "movq" <+> pr r0 <> "," <+> pr r1
      pp (IAddRR _ r0 r1)              = "add" <+> pr r0 <> "," <+> pr r1
      pp (IAddRI _ r i)                = "add" <+> pr r <> "," <+> decimal i
      pp (ISubRR _ r0 r1)              = "sub" <+> pr r0 <> "," <+> pr r1
      pp (ISubRI _ r i)                = "sub" <+> pr r <> "," <+> decimal i
      pp (IMulRR _ r0 r1)              = "imul" <+> pr r0 <> "," <+> pr r1
      pp (IMulRA _ r a)                = "imul" <+> pr r <> "," <+> pr a
      pp (Jcc _ p l)                   = "j" <> pr p <+> prLabel l
      pp Ret{}                         = "ret"
      pp (Vdivsd _ rD r0 r1)           = "vdivsd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Movapd _ r0 r1)              = "movapd" <+> pr r0 <> "," <+> pr r1
      pp (Cvttsd2si _ r0 r1)           = "cvttsd2si" <+> pr r0 <> "," <+> pr r1
      pp (Vmulsd _ rD r0 r1)           = "vmulsd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vaddsd _ rD r0 r1)           = "vaddsd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (VaddsdA _ rD r a)            = "vaddsd" <+> pr rD <> "," <+> pr r <> "," <+> pr a
      pp (Vsubsd _ rD r0 r1)           = "vsubsd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Cvtsi2sd _ r0 r1)            = "cvtsi2sd" <+> pr r0 <> "," <+> pr r1
      pp (Roundsd _ r0 r1 m)           = "roundsd" <+> pr r0 <> "," <+> pr r1 <> "," <+> pr m
      pp (CmpRI _ r i)                 = "cmp" <+> pr r <> "," <+> decimal i
      pp (Divsd _ r0 r1)               = "divsd" <+> pr r0 <> "," <+> pr r1
      pp (Mulsd _ r0 r1)               = "mulsd" <+> pr r0 <> "," <+> pr r1
      pp (Addsd _ r0 r1)               = "addsd" <+> pr r0 <> "," <+> pr r1
      pp (Subsd _ r0 r1)               = "subsd" <+> pr r0 <> "," <+> pr r1
      pp (MovRA _ r a)                 = "mov" <+> pr r <> "," <+> pr a
      pp (MovAR _ a r)                 = "mov" <+> pr a <> "," <+> pr r
      pp (MovAI32 _ a i)               = "mov qword" <+> pr a <> "," <+> decimal i
      pp (MovqXA _ x a)                = "movq" <+> pr x <> "," <+> pr a
      pp (MovqAX _ a x)                = "movq" <+> pr a <> "," <+> pr x
      pp (Fld _ a)                     = "fld qword" <+> pr a
      pp Fyl2x{}                       = "fyl2x"
      pp (Fstp _ a)                    = "fstp qword" <+> pr a
      pp F2xm1{}                       = "f2xm1"
      pp Fldl2e{}                      = "fldl2e"
      pp Fldln2{}                      = "fldln2"
      pp Fld1{}                        = "fld1"
      ...
      pp Fscale{}                      = "fscale"
      pp Fninit{}                      = "fninit"
      pp (Fxch _ st)                   = "fxch" <+> pr st
      pp (FldS _ st)                   = "fld" <+> pr st
      pp Fmulp{}                       = "fmulp"
      pp (Vfmadd231sd _ rD r0 r1)      = "vfmadd231sd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vfmnadd231sd _ rD r0 r1)     = "vfmnadd231sd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vfmadd213sd _ rD r0 r1)      = "vfmadd213sd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vfmsub231sd _ rD r0 r1)      = "vfsubd231sd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vfmsub213sd _ rD r0 r1)      = "vfsubd213sd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vfmsub132sd _ rD r0 r1)      = "vfsubd132sd" <+> pr rD <> "," <+> pr r0 <> "," <+> pr r1
      pp (Vfmadd231sdA _ rD r a)       = "vfmadd231sd" <+> pr rD <> "," <+> pr r <> "," <+> pr a
      pp (Push _ r)                    = "push" <+> pr r
      pp (Pop _ r)                     = "pop" <+> pr r
      pp (IDiv _ r)                    = "idiv" <+> pr r
      pp (Call _ f)                    = "call" <+> pr f <+> "wrt ..plt"
      pp (Sal _ r i)                   = "sal" <+> pr r <> "," <+> decimal i
      pp (Sar _ r i)                   = "sar" <+> pr r <> "," <+> decimal i
      pp (Sqrtsd _ r0 r1)              = "sqrtsd" <+> pr r0 <> "," <+> pr r1
      pp (Maxsd _ r0 r1)               = "maxsd" <+> pr r0 <> "," <+> pr r1
      pp (Vmaxsd _ r0 r1 r2)           = "vmaxsd" <+> pr r0 <> "," <+> pr r1 <> "," <+> pr r2
      pp (VmaxsdA _ r0 r1 a)           = "vmaxsd" <+> pr r0 <> "," <+> pr r1 <> "," <+> pr a
      pp (Minsd _ r0 r1)               = "minsd" <+> pr r0 <> "," <+> pr r1
      pp (Vminsd _ r0 r1 r2)           = "vminsd" <+> pr r0 <> "," <+> pr r1 <> "," <+> pr r2
      pp (Not _ r)                     = "not" <+> pr r
      pp (And _ r0 r1)                 = "and" <+> pr r0 <> "," <+> pr r1
      pp (Cmov _ p r0 r1)              = "cmov" <> pm p <+> pr r0 <> "," <+> pr r1
      pp (Rdrand _ r)                  = "rdrand" <+> pr r
      pp (Test _ r0 r1)                = "test" <+> pr r0 <> "," <+> pr r1
      pp (TestI _ r0 i)                = "test" <+> pr r0 <> "," <+> decimal i
      pp (MovqRX _ r xr)               = "movq" <+> pr r <> "," <+> pr xr
      pp (Vcmppd _ xr0 xr1 xr2 Eqoq)   = "vcmpeqpd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Ltos)   = "vcmpltpd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Leos)   = "vcmplepd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Unordq) = "vcmpunordpd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Nequq)  = "vcmpneqpd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Nltus)  = "vcmpnltpd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Nleus)  = "vcmpnlepd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (Vcmppd _ xr0 xr1 xr2 Ordq)   = "vcmpordpd" <+> pr xr0 <> "," <+> pr xr1 <> "," <+> pr xr2
      pp (C _ l)                       = "call" <+> prLabel l
      pp RetL{}                        = "ret"
      pp (Neg _ r)                     = "neg" <+> pr r
