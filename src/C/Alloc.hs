...
iF a = gg where
    ...
    gg [s@(WT l _ cs)]                       = s { body = gg cs }:fss l
    ...
    gg (s@(For l _ _ _ _ _ _ cs):ss@(s0:_))  = s { body = gg cs }:fs l s0++gg ss
    gg (s@(While l _ _ _ cs):ss@(s0:_))      = s { body = gg cs }:fs l s0++gg ss
    gg (s@(WT l _ cs):ss@(s0:_))             = s { body = gg cs }:fs l s0++gg ss
    gg (s@(Rof l _ _ _ cs):ss@(s0:_))        = s { body = gg cs }:fs l s0++gg ss
