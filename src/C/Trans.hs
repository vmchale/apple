...
writeF (Lam _ x e) (FA xr:rs) ret = addD x xr *> writeF e rs ret
...
wt p (PT t) = WrP () p (Is t); wt p (ΠT rs) = WrT () p (rp<$>rs)
...
rfill (Builtin _ ConsE) (AD t lA _ _ _ _) [NA (ΠT ps), AI (AD xsR lX _ _ (Just sz) (Just n))] = do
    pure [WrT () (AElem t 1 lA 0 sz) (rp<$>ps), cpy (AElem t 1 lA 1) (AElem xsR 1 lX 0) n sz]
...
rfill (Builtin _ Snoc) (AD t lA _ _ _ _) [NA (ΠT ps), AI (AD xsR lX _ _ (Just sz) (Just n))] = do
    pure [WrT () (AElem t 1 lA n sz) (rp<$>ps), cpy (AElem t 1 lA 0) (AElem xsR 1 lX 0) n sz]
...
aeval (EApp oTy@(Arr oSh _) (EApp _ (Builtin _ Del) x) j) t a | Just (tX, rnk) <- tRnk (eAnn x), Just sz <- nSz tX = do
    nO <- nI; nE <- nI
...
aeval (EApp _ (EApp _ (Builtin _ Ices) p) xs) t a | Arrow tX _ <- eAnn p, tXs@(Arr sh _) <- eAnn xs, Just sz <- nSz tX = do
    szR <- nI; nR <- nI; b <- nBT
    (plX, (lX, xsR)) <- plA xs
    (xR, rX) <- arg tX (iXelem xsR 1 lX sz)
    ss <- writeRF p [xR] (PT b)
...
aeval (EApp (Arr oSh _) (EApp _ (Builtin _ Map) f) xs) t a
    ...
    (y, wRet) <- rW tC (iXelem t 1 (Just a) sz)
...
aeval (EApp (Arr oSh _) (EApp _ (EApp _ (Builtin _ Outer) op) xs) ys) t a
    ...
    (plX, (lX, xR)) <- plA xs; (plY, (lY, yR)) <- plA ys
    (x, wX) <- arg tX (iXelem xR 1 lX szXT)
    (y, wY) <- arg tY (iXelem yR 1 lY szYT)
...
aeval (EApp (Arr oSh _) (EApp _ (EApp _ (Builtin _ Fib) seed) op) n) t a | Just ty <- aN tSeed, sz <- bT ty = do
    ...
    (y, wRet) <- rW ty (iXelem t 1 (Just a) sz)
...
eval (EApp _ (EApp _ (Builtin _ IOf) p) xs) t | (Arrow tD _) <- eAnn p, Just szX <- nSz tD = do
    ...
    szR <- nI; i <- nI; done <- nI
    (plX, (lX, xsR)) <- plA xs
    (x, wX) <- arg tD (iXelem xsR 1 lX szX)
