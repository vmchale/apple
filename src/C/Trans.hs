...
data AD = AD { eigen :: !Temp, alabel :: !(Maybe AL), eiSh :: Maybe (Sh ())
             , arnk :: Maybe CE
             , elemSz :: Maybe Int64
             , arrn :: (Maybe CE)
             }

fill :: E (T ()) -> AD -> [(Temp, Maybe AL)] -> CM [CS ()]
...
fill (Builtin (Arr sh I) Eye) (AD t lA _ (Just rnk) _ _) [] | Just [i,_] <- staIx sh = do
    ...
    pure [td=:DP t rnk, loop]
fill (Builtin _ Init) (AD t lA _ _ (Just sz) (Just n)) [(xR,lX)] =
    pure [CpyE () (AElem t 1 0 lA sz) (AElem xR 1 0 lX sz) n sz]
fill (Builtin _ Tail) (AD t lA _ _ (Just sz) (Just n)) [(xR,lX)] =
    pure [CpyE () (AElem t 1 0 lA sz) (AElem xR 1 1 lX sz) n sz]
fill (EApp _ (Builtin _ Map) op) (AD t lA (Just sh) _ _ (Just n)) [(xR,l)] | (Arrow tD tC) <- eAnn op, nind tD = do
    iR <- nI; xRd <- nI; td <- nI;
...
aeval (EApp oTy@(Arr oSh _) e@(Builtin _ Init) x) t | Just sz <- aB oTy = do
    ...
    (plX, (lX, xR)) <- plA x
    contents <- fill e (AD t (Just a) Nothing Nothing (Just sz) (Just$Tmp nR)) [(xR, lX)]
...
aeval (EApp _ f@(EApp _ (Builtin _ Map) op) e) t | tX@(Arr sh _) <- eAnn e, (Arrow tD tC) <- eAnn op, Just sz <- nSz tC, nind tD = do
    ...
    contents <- fill f (AD t (Just a) (Just sh) Nothing Nothing (Just$Tmp nR)) [(xR,l)]
...
aeval (EApp (Arr oSh _) (EApp _ (Builtin _ CatE) x) y) t | tX <- eAnn x, Just (ty, 1) <- tRnk tX = do
    xnR <- nI; ynR <- nI; tn <- nI
    let tyN=bT ty
...
aeval (EApp (Arr sh _) (EApp _ (EApp _ (Builtin _ IRange) start) end) incr) t = do
    ...
    let pN=n =: (Bin Op.IDiv (Tmp endR - Tmp startR) (Tmp incrR)+1)
        loop=for sh i 0 ILt (Tmp n) [Wr () (AElem t 1 (Tmp i) (Just a) 8) (Tmp startR), startR+=Tmp incrR]
...
aeval (EApp (Arr oSh _) (EApp _ (EApp _ (Builtin _ Outer) op) xs) ys) t | (Arrow tX (Arrow tY tC)) <- eAnn op, Just zSz <- nSz tC, nind tX && nind tY = do
    ...
    let loop=fort (eAnn xs) i 0 ILt (Tmp szX) [fort (eAnn ys) j 0 ILt (Tmp szY) (step++[k+=1])]
    pure (Just a, plX$plY$szX =: ev (eAnn xs) (xR,lX):szY =: ev (eAnn ys) (yR,lY):Ma () oSh a t 2 (Tmp szX*Tmp szY) zSz:diml (t, Just a) [Tmp szX, Tmp szY]++k=:0:sas pinches [loop])
