...
cmm = fmap (f.C.writeC).opt where f (cs,_,aa,_)=(lift cs,aa)

ec :: Int -> E a -> Either (Err a) ([CS], LSt, C.AsmData, IM.IntMap C.Temp)
ec i = fmap (g . C.writeC) . optE i where g (cs,x,y,z)=(lift cs,x,y,z)

ir :: BSL.ByteString -> Either (Err AlexPosn) ([Stmt], IR.AsmData, WSt)
ir = fmap (f.C.writeC).opt where f (cs,u,aa,t) = let (s,u')=cToIR u (lift cs) in (frees (ctemp<$>t) (optIR s),aa,u')

eir :: Int -> E a -> Either (Err a) ([Stmt], IR.AsmData, WSt)
eir i = fmap (f.C.writeC).optE i where f (cs,u,aa,t) = let (s,u')=cToIR u (lift cs) in (frees (ctemp<$>t) (optIR s),aa,u')
