...
Array languages such as APL and J suffer from undeserved obscurity \cite{hsu2023}. In order to make them more useful to enthusiasts, one might hope to embed them. % , providing users with the facilities of libraries such as matplotlib.
...
But this imposes on the calling code, requiring care to increment the reference count and % also delete a GC?
% But this means that arrays depend on a garbage collector running alongside to behave sensibly.
...
Our work is based on established liveness algorithms; we define {\tt uses} and {\tt defs} on statements and thence compute liveness information \cite[pp.~213-216]{appel1998}

live intervals \cite{poletto1999} for arrays.
% cite appel
...
\begin{tikzpicture}[auto]
    ...
    \node [block, below of=plain] (live) {\tt [(Stmt, Live)]};
