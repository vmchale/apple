...
% make it more equal in embedding (like C)
...
\end{verbatim}
performs one allocation for each {\tt irange} but this could be consolidated into one---{\tt irange 0 20 1} is inferred to have type {\tt Vec 20 int} in the existing compiler; with liveness and size information one could allot arrays which are not live at the same time to % do something like linear register allocation in which arrays are assigned to memory slots.

% Possible because we are in a high-level language
% rewrite: thrust is "thus constrained arrays are an interesting topic of study"
