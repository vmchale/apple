...
But this imposes on the calling code, requiring care to increment the reference count % also delete a GC?
...
All elements are flat; there are only two primitive types: 64-bit integers and 64-bit floats. Arrays of tuples are supported but not arrays of tuples of arrays; there are no user-defined types.
...
Our work is based on established liveness algorithms; we annotate statements with {\tt uses} and {\tt defs} and thence compute live intervals \cite{poletto1999} for arrays.
% cite appel
