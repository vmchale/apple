...
\begin{abstract}
    ...
    Here we present statically determined memory allocation used in the Apple array system, a JIT compiler. Arrays are usable as pointers, without depending on a garbage collector. % procedures too...
...
Array languages such as APL and J suffer from a lack of popularity, baffling their proponents \cite{hsu2023}. % look at C instead!

C seems vulgar in offering pointers as arrays; APL dialects

% cite Hui: 12 (.net fail)

% \cite{chakravarty2011}
% cite NumPy paper "API for scientific computing"

% Kell paper (low-level, equate pointers with arrays \cite{munksgaard2022} (rather than environmental constraints such as relation/dependence on garbage collector))

% full language incl.
% array languages fail to export their constructs (trains)

% Array libraries like NumPy take inspiration from J, but cannot offer the
% Our approach embeds a full language. Procedures can be called from a variety of languages (C, Python, Haskell) with the same type system---specialized for arrays---without compromising in order to accommodate Python's lack of static typing or C's lack of sophistication.
...
This may look spartan to functional programmers, but it is enough to
...
There is no guarantee that freeing at the end of a live interval guarantees that all {\tt free} statements are reachable, but since we control IR generation, we can check that this is indeed the cases in loops and conditionals.
...
This is a further advantage of completely flat arrays. That one can get such performance without reference counting
